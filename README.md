...
```
python build/export_free_pins.py
```

เขียน `block-free-output-pins.xlsx` ไว้ที่โฟลเดอร์ `FOXBORO` (นอก repo — 28 MB
ไม่ควรอยู่ใน git) · ใช้ `block_params.json` (ตารางขาของ B0193AX) คู่กับ `graph.js`
(สายทั้งหมด รวมสายพารามิเตอร์ ฮาร์ดแวร์ ECB และ Jove)

**เอาเฉพาะขา OUTPUT** ที่ต่อสายได้ (`con = true`) แต่ไม่มี edge ออกจากขานั้น —
**487,364 ขา จาก 57,641 บล็อก** และ **24,831 บล็อกที่ไม่มีขาออกไหนถูกอ่านเลย**
(ฝั่ง input มีอีก 427,142 แถวแต่แทบไม่มีความหมาย — AIN มีขาเข้าต่อได้ 56 ขา
เพราะตั้งค่าได้ ไม่ใช่เพราะควรมี 56 สาย)

**คอลัมน์ `สัดส่วนที่ต่อ` คือหัวใจของไฟล์นี้** — B0193AX เรียงขาออกตามตัวอักษร
(AIN คือ BAD, CRIT, HAI, HHAIND, … , PNT) ลำดับขาจึงไม่บอกว่าขาไหนสำคัญ
คอลัมน์นี้นับว่า *บล็อกชนิดเดียวกันทั้งโรงงานต่อขานี้กี่ตัว* แยกของจริงออกจาก noise
โดยไม่ต้องเดาว่าชื่อขาไหนดูสำคัญ

- สัดส่วน ≈ 0 → ทั้งโรงงานก็ไม่มีใครต่อขานี้ (เช่น `AIN.HAI`) เป็นเรื่องปกติของชนิดบล็อก
  · **388,963 แถว (80%) อยู่กลุ่มนี้**
- สัดส่วนสูง → พี่น้องชนิดเดียวกันต่อกันเกือบหมด แต่ตัวนี้ไม่ต่อ ← ของจริง
  · เหลือแค่ **2,701 แถวที่ ≥ 0.5** และ **170 แถวที่ ≥ 0.9**

ชีตแรกของไฟล์คือ **`ขาที่น่าสงสัย`** (2,701 แถว เรียงมาให้แล้ว) เปิดมาเจอเลย
ไม่ต้องไปหาเองในตาราง 487,364 แถว · ตัวอย่างที่ขึ้นบนสุด

| บล็อก | TYPE | ขา | พี่น้องที่ต่อ |
|---|---|---|---|
| `CBM_A:32HV001` | MOVLV | `COUT_1` · `COUT_2` · `MMAIND` | 764 / 765 |
| `M3307:33PIC522` ฯลฯ (4 ตัว) | PIDA | `OUT` | 241 / 245 |
| `C604:06XCA071` | RATIO | `OUT` | 36 / 37 |

ชีตอื่น: `ขาออกไม่มีคนอ่าน` (ครบทุกแถว) · สรุปตาม TYPE / CP / AREA · `หมายเหตุ`

**ข้อจำกัดที่เขียนไว้ในชีต หมายเหตุ ด้วย**

- บล็อก 4,607 ตัวที่ TYPE ไม่มีตารางขาใน `block_params.json` **ไม่อยู่ในไฟล์เลย**
  (LONG 3,207 · UNIVFF 893 · AI 379 · AO 69 · STA 41 · PIDFF 14 · MAO 2 · COMPND 2)
  ไม่มีรายการขา = ไม่มีอะไรให้ตรวจ ไม่ใช่ว่าสะอาด
- JOVE object (18,502) และ ECB (1,947) ไม่อยู่โดยตั้งใจ — ไม่ใช่บล็อกที่มีตารางขาแบบ B0193AX
- **สายจากโปรแกรม sequence (IND) อยู่ใน `sequence.js` ไม่ได้อยู่ใน `graph.js`**
  ขาที่ถูกอ่านโดยโปรแกรม sequence เท่านั้นจึงยังโผล่มาในไฟล์นี้ได้ —
  ก่อนลบอะไรที่เกี่ยวกับ IND ให้เช็ค Sequence View ก่อน
- `RIN.OUTPUTS` / `RINR.OUTPUTS` เป็นหัวข้อในคู่มือที่ตัวแกะ PDF อ่านมาเป็นชื่อขา
  ตัดออกแล้ว ไม่งั้นจะได้แถวปลอมหนึ่งแถวต่อบล็อก RIN หนึ่งตัว
